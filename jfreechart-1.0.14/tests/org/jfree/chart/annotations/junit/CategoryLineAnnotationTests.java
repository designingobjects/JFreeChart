...
public class CategoryLineAnnotationTests extends TestCase {
	...
	public void testConstructor_IllegalArguments() {
		try {
			new CategoryLineAnnotation(null, 1.0, "Category 2", 2.0, Color.red,
					new BasicStroke(1.0f));
			fail();
		} catch (IllegalArgumentException e) {
		}
		try {
			new CategoryLineAnnotation("Category 1", 1.0, null, 2.0, Color.red,
					new BasicStroke(1.0f));
			fail();
		} catch (IllegalArgumentException e) {
		}
		...
		try {
			new CategoryLineAnnotation("Category 1", 1.0, "Category 2", 2.0,
	...
	public void testSetters_IllegalArguments() {
		CategoryLineAnnotation a = new CategoryLineAnnotation("Category 1",
				1.0, "Category 2", 2.0, Color.red, new BasicStroke(1.0f));
		try {
			a.setCategory1(null);
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals("Category 1", a.getCategory1());
		}
		try {
			a.setCategory2(null);
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals("Category 2", a.getCategory2());
		}
		...
		try {
			...
			assertNotNull(a.getStroke());
		}
		try {
			a.setPaint(null);
			fail();
		} catch (IllegalArgumentException e) {
			assertNotNull(a.getPaint());
		}
	}
	...
	public void testDrawNullInfo() {
		...
		try {
			...
			CategoryDataset dataset = DatasetUtilities.createCategoryDataset(
					"", "", data);
			CategoryLineAnnotation a = new CategoryLineAnnotation("1", 1.0,
					"2", 2.0, Color.red, new BasicStroke(1.0f));
		...
		try {
			...
			JFreeChart chart = ChartFactory.createBarChart("Bar Chart",
					"Domain", "Range", dataset, PlotOrientation.VERTICAL, true, // include
																				// legend
					true, true);
			CategoryPlot plot = (CategoryPlot) chart.getPlot();
			plot.addAnnotation(a);
